...
def convert(inputs):
	...
	if (rname != "*") and ("M" in cigar) and ("H" not in cigar): # aligned and non-hard-clipped reads
		...
		sf = str(s5)+"_"+str(s3)
		# aligned read sequence info
		aligned_seq = get_aligned_read_sequence(cigar,seq)
		# exon info
		cigar_n_l = 0
		exon_length = 0
		...
		gpd_output = "\t".join([qname,qname,rname,strand,str(int(pos)-1),str(exon_end),mapq,sf,str(len(exon_start_list)-1),",".join(exon_start_list),",".join(exon_end_list),flag,aligned_seq])
	else:
		gpd_output = None
	return gpd_output
